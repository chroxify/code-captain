...
struct MessageBubbleView: View {
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: message.displayIsFromUser ? .trailing : .leading, spacing: 8) {
                HStack {
                    if !message.displayIsFromUser {
...
struct ToolUseView: View {
    ...
    private func formatToolInput() -> String? {
        ...
        case .grep:
            if let pattern = toolUse.input["pattern"]?.value as? String {
                let outputMode = toolUse.input["output_mode"]?.value as? String ?? "files_with_matches"
        ...
        case .edit:
            if let filePath = toolUse.input["file_path"]?.value as? String {
                let oldString = toolUse.input["old_string"]?.value as? String ?? "content"
                return "Editing: \(filePath) - \(oldString.prefix(30))..."
            }
            
        case .multiEdit:
            if let filePath = toolUse.input["file_path"]?.value as? String {
                let edits = toolUse.input["edits"]?.value as? [[String: Any]] ?? []
        ...
        case .notebookRead:
            if let notebookPath = toolUse.input["notebook_path"]?.value as? String {
                return "Reading notebook: \(notebookPath)"
            }
            
        case .notebookEdit:
            if let notebookPath = toolUse.input["notebook_path"]?.value as? String {
                let editMode = toolUse.input["edit_mode"]?.value as? String ?? "replace"
        ...
        case .fileRead, .fileWrite, .fileCreate, .fileDelete, .fileMove, .fileCopy:
            if let filePath = toolUse.input["file_path"]?.value as? String {
        ...
        // Git Operations
        case .gitStatus, .gitAdd, .gitCommit, .gitPush, .gitPull, .gitBranch, .gitCheckout, .gitMerge, .gitDiff, .gitLog:
        ...
        // Code Analysis
        case .codeAnalysis, .syntaxCheck, .lintCheck, .formatCode, .findReferences, .findDefinition:
            if let filePath = toolUse.input["file_path"]?.value as? String {
        ...
        case .installDependencies:
            if let packageManager = toolUse.input["package_manager"]?.value as? String {
    ...
    private func formatGenericInput() -> String? {
        ...
        if let command = toolUse.input["command"]?.value as? String {
            ...
            return "Search: \(query)"
        } else if let description = toolUse.input["description"]?.value as? String {
            return description
...
struct ToolResultView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: toolResult.is_error == true ? "xmark.circle" : "checkmark.circle")
        ...
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
        .background((toolResult.is_error == true ? Color.red : Color.green).opacity(0.1))
        .cornerRadius(16)
    }
...
struct UserToolResultView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: block.is_error == true ? "xmark.circle" : "checkmark.circle")
        ...
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
        .background((block.is_error == true ? Color.red : Color.green).opacity(0.1))
        .cornerRadius(16)
    }
...
struct ResultMessageView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: resultMessage.is_error ? "xmark.circle" : "checkmark.circle")
            ...
            if isExpanded {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack {
                            Text("Total Duration:")
                                .font(.caption)
                                .foregroundColor(.secondary)
                            Text("\(String(format: "%.2f", resultMessage.duration_ms / 1000))s")
                                .font(.caption)
                                .fontDesign(.monospaced)
                                .foregroundColor(.primary)
                        }
                        ...
                        HStack {
                            Text("API Duration:")
                                .font(.caption)
                                .foregroundColor(.secondary)
                            Text("\(String(format: "%.2f", resultMessage.duration_api_ms / 1000))s")
                                .font(.caption)
                                .fontDesign(.monospaced)
                                .foregroundColor(.primary)
                        }
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack {
                            ...
                            Text("Total Cost:")
                                .font(.caption)
                                .foregroundColor(.secondary)
                            Text("$\(String(format: "%.4f", resultMessage.total_cost_usd))")
                                .font(.caption)
                                .fontDesign(.monospaced)
                                .foregroundColor(.primary)
                        }
                                ...
                                if let serverToolUse = usage.server_tool_use {
                                    HStack {
                                        Text("Web Searches:")
                                            .font(.caption)
                                            .foregroundColor(.secondary)
                                        Text("\(serverToolUse.web_search_requests)")
                                            .font(.caption)
                                            .fontDesign(.monospaced)
                                            .foregroundColor(.primary)
                                    }
                ...
                HStack {
                    Text("Duration: \(String(format: "%.2f", resultMessage.duration_ms / 1000))s")
                        .font(.caption)
                        .foregroundColor(.secondary)
        ...
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
        .background((resultMessage.is_error ? Color.red : Color.green).opacity(0.1))
        .cornerRadius(16)
    }
}
